...
def remove_cross_corpus_duplicates(data):
	...
	... 	{'fragment': Decitala("Pratitala"), 'mod': ('d', 2.0), 'onset_range': (0.5, 1.0), 'is_spanned_by_slur' : False, 'pitch_content': [(91,), (78,), (85,)], "id":6} # noqa
	... ]
	...
	{'fragment': <fragment.GeneralFragment cs-test1: [0.25 0.25]>, 'mod': ('cs', 2.0), 'onset_range': (0.0, 0.5), 'is_spanned_by_slur': False, 'pitch_content': [(80,), (91,)], 'id': 2} # noqa
	{'fragment': <fragment.GreekFoot Trochee>, 'mod': ('r', 0.125), 'onset_range': (0.25, 0.625), 'is_spanned_by_slur': False, 'pitch_content': [(91,), (78,)], 'id': 3}
	{'fragment': <fragment.GeneralFragment cs-test2: [0.25  0.125]>, 'mod': ('cs', 2.0), 'onset_range': (0.25, 0.625), 'is_spanned_by_slur': False, 'pitch_content': [(80,), (91,)], 'id': 4} # noqa
...
def _prepare_fragment_data(
	...
	for this_frag_type in frag_types:
		...
		if "ratio" in rep_types:
			curr_ratio_tree = FragmentTree.from_frag_type(
				frag_type=this_frag_type,
				rep_type="ratio"
			)
		if "difference" in rep_types:
			curr_difference_tree = FragmentTree.from_frag_type(
				frag_type=this_frag_type,
				rep_type="difference"
			)
		...
		for path in pareto_optimal_paths:
			...
			for this_fragment_data in path:
				...
				for j, this_row in enumerate(fragment_rows, start=1):
					if (this_row[2] == data["fragment"].name) and (this_row[0] == data["onset_range"][0]) and (this_row[1] == data["onset_range"][1]): # noqa
...
def create_database(
	...
	"""
	This function generates an sqlite3 database for storing extracted rhythmic data from
	:obj:`decitala.trees.rolling_search`. The database holds has one page for storing all
	extracted fragments, their onsets/offsets of occurrence, and their modification data.
	...
	:param bool filter_single_anga_class: whether or not to remove single-anga class fragments
										from the data in rolling_search.
	:param bool filter_found_sub_fragments: whether to remove sub-fragments from the dataset.
	:param bool verbose: whether or not to log information about the database creation process
						(including data from rolling search).
	...
	fragment_table = _make_fragment_table(
		name="UnfilteredFragments",
		data=sorted_onset_ranges_two,
		metadata=metadata
	)
...
class DBParser:
	...
	def slurred_fragments(self):
		...
		:rtype: list
		"""
		return [x for x in self.fragment_data if x["is_slurred"] is True]
	...
	def show_subpath(self, table_num, path_num):
		...
		:rtype: pandas.DataFrame
		"""
		assert (table_num >= 1 and path_num >= 1), DatabaseException("The table_num and path_num must be >= 1.") # noqa
	...
	def subpath_onset_percentile(self, table_num, path_num):
		"""
		Given the data from
		:obj:`~decitala.database.DBParser.table_average_onsets_per_fragment_per_subpath`, gets the
	...
	def first_subpath_start_gap_score(self, path_num):
		"""
		:param int path_num: 1-indexed subpath number.
		:return: the proportion of the starting gap (i.e. the empty region from the earliest
				detected onset in the table to the start of the queried subpath) to the total
				range of the queried subpath in the first table.
		:rtype: float
		"""
	...
	def subpath_intra_gap_score(self, table_num, path_num):
		...
		:param int table_num: 1-indexed table number.
		:param int path_num: 1-indexed subpath number.
		:return: proportion of the total gaps between fragments in a subpath to the total range of
				the subpath.
		:rtype: float
		"""
		assert (table_num >= 1 and path_num >= 1), DatabaseException("The table_num and path_num must be >= 1.") # noqa
	...
	def intra_subpath_model_score(
		...
		assert (table_num >= 1 and path_num >= 1), DatabaseException("The table_num and path_num must be >= 1.") # noqa
		if table_num == 1:
			assert len(weights) == 3, DatabaseException("For the first table, three weights must be provided.") # noqa
	...
	def model_full_path(
		...
		path = []
		table_num = 1
		with Bar("Modeling full path...", max=self.metadata[-1][0] + 1, check_tty=False, hide_cursor=False) as bar: # noqa the latter two arguments allow for displaying in jupyter.
			while table_num < self.metadata[-1][0] + 1:
				if table_num == 1:
					...
					for row_num in range(1, curr_data[1] + 1):
						...
						combined = (intra_inter_weights[0] * intra_model_score) + (intra_inter_weights[1] * inter_subpath_score) # noqa
...
def _get_initial_data(path, size, log_msg):
	...
	with Bar(log_msg, max=size) as bar:
		for this_file in os.listdir(path):
			...
			ql_array = [this_note.quarterLength for this_note in converted.flat.getElementsByClass(note.Note)] # noqa
...
def _make_table(data, table_name, conn):
	...
	table_string = "CREATE TABLE {} (Name BLOB, QL_Array BLOB, R_Equivalents BLOB, D_Equivalents BLOB, R_Keep INT, D_Keep INT)".format(table_name) # noqa
	...
	with Bar("Making {} Table...".format(table_name), max=len(data)) as bar:
		for this_fragment_data in data:
			...
			fragment_insertion_string = "INSERT INTO {0} VALUES('{1}', ?, ?, ?, ?, ?)".format(table_name, name) # noqa
			conn.cursor().execute(fragment_insertion_string, (str(ql_array), str(r_equivalents), str(d_equivalents), r_keep, d_keep)) # noqa
			bar.next()
...
def _create_fragment_database(verbose=True):
	...
	decitala_initial_data = _get_initial_data(
		path=decitala_path,
		size=130,
		log_msg="Getting initial decitala data..."
	)
	greek_metric_initial_data = _get_initial_data(
		path=greek_path,
		size=26,
		log_msg="Getting initial greek metric data..."
	)
	...
	final_decitala_data = _track_keep_vals(
		data=decitala_cross_equivalents,
		size=130,
		frag_type="decitala"
	)
