...
class NaryTree:
	"""
	A single-rooted nary tree for ratio and difference representations of rhythmic fragments. Nodes
	are hashed by their value and are stored in a set. For demonstration, we will create the
	...
	>>> # We can serialize an NaryTree as either a native Python type or Javascript type.
	>>> TestTree.serialize()
...
def filter_data(raw_data):
	...
	:param list raw_data: a list of :class:`~decitala.fragment.Decitala`,
						:class:`~decitala.fragment.GreekFoot`, or
		...
		for j, _ in enumerate(copied):
			try:
				if i == j:
					...
					pass
				elif cauchy_schwartz(copied[i].ql_array(), copied[j].ql_array()) is True:
					pass
				elif cauchy_schwartz(copied[i].ql_array(), copied[j].ql_array()) is False:
